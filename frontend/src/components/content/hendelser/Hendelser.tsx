...
import {hendelseprefix, skjulPåminnelserState, snuPåHendelseListenState, visBareFeilState} from '../../../state/state'
...
export const Hendelser = ({ hendelser }: { hendelser: Hendelsekontekst[] }) => {
    ...
    const [snuPåListen, setSnuPåListen] = useAtom(snuPåHendelseListenState)
    ...
    const toggleSnuPåListen = () => setSnuPåListen(!snuPåListen)
    const harRettPrefix = (kontekst: Hendelsekontekst) =>
        kontekst.kontekstType.toLowerCase().startsWith(prefix.toLowerCase())
    ...
    const sorterteHendelser = snuPåListen
        ? filtrerteHendelser.sort((a, b) => compareAsc(a.opprettet, b.opprettet))
        : filtrerteHendelser.sort((a, b) => compareAsc(b.opprettet, a.opprettet))
    ...
    return (
        <Box background="default">
            <HStack gap="space-16">
                ...
                <Switch size="small" checked={snuPåListen} onChange={(_) => toggleSnuPåListen()}>Snu på listen</Switch>
